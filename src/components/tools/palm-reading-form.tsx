...
import { Loader2, Camera, Upload, Hand, RefreshCw } from 'lucide-react';
  ...
  return (
    <Button type="submit" disabled={pending || disabled} className="w-full">
      {pending ? (
        ...
        <>
          <Hand className="mr-2 h-4 w-4" /> Analyze My Palm
        </>
      )}
    </Button>
  );
...
export function PalmReadingForm() {
    const [state, formAction] = useActionState(palmReadingAction, initialState);
            ...
            <form action={formAction}>
                ...
                <div className="space-y-4">
                            ...
                            <CardContent className="p-2">
                                <div className="relative w-full aspect-video rounded-md overflow-hidden bg-muted">
                                    ...
                                    <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                                        <div className="w-[70%] h-[80%] border-4 border-dashed border-primary/70 rounded-xl bg-black/10 flex items-center justify-center">
                                             <Hand className="text-primary/20 w-32 h-32" />
                                        </div>
                    ...
                    {imagePreview && (
                        <Card>
                            <CardContent className="p-2">
                                <Image src={imagePreview} alt="Preview" width={500} height={400} className="rounded-md w-full object-contain max-h-80" data-ai-hint="hand palm" />
                                <div className="grid grid-cols-2 gap-2 mt-2">
                    ...
                    <CardDescription className="text-center text-xs">
                        Your privacy is important. Photos are not stored after analysis. For best results, use a clear, well-lit picture of your dominant hand.
                    </CardDescription>
                </div>
            </form>
        </div>
